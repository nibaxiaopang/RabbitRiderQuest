//
//  RabbitRiderPravicyViewController.swift
//  RabbitRiderQuest
//
...
class RabbitRiderPravicyViewController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
    ...
    var privacyData: [Any]?
    @objc var url: String?
    let rabbitRiderPrivacyPolicyUrl = "https://www.termsfeed.com/live/935ef5f7-aa1e-4dd5-9730-d77689a59493"
    
    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.rabbitRiderGetUserDefaultKey())
        initSubViews()
        initNavView()
        initWebView()
        rabbitRiderStartLoadWebView()
    }
    ...
    */
    ...
    private func rabbitRiderStartLoadWebView() {
        let urlStr = url ?? rabbitRiderPrivacyPolicyUrl
    ...
    private func rabbitRiderReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "RabbitRiderPravicyViewController") as? RabbitRiderPravicyViewController {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                rabbitRiderSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                rabbitRiderReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        rabbitRiderSendEvent(tName, values: [tName: data])
                    }
                } else {
                    rabbitRiderSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                rabbitRiderAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    rabbitRiderAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = rabbitRiderJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = rabbitRiderJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    rabbitRiderSendEvents(withParams: evParams)
                }
            }
        }
    }
